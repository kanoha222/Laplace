...
从 SKILL_REGISTRY 和领域知识生成 OpenAI tools JSON 定义。
...
def build_agent_tools() -> list[dict]:
    ...
    注意：dashscope Responses API 不支持多轮 tool 调用，
    请使用 build_agent_tools_chat() 获取 Chat Completions 格式。
    """
    return [t["function"] for t in build_agent_tools_chat()]


def build_agent_tools_chat() -> list[dict]:
    """构建 Chat Completions API 格式的 tools 定义列表。
...
def _tool_lookup_servant() -> dict:
    return {
        ...
        "function": {
            "name": "lookup_servant",
            "description": (
                "按名称查询单个从者的详细信息。支持中文名、英文名、日文名和昵称。如'查一下梅林'、'玛修的详情'"
            ),
            "parameters": {
                "type": "object",
                "properties": {
