        ...
        messages.append(
            {
                "role": "system",
                "content": (
                    "【前置信息】系统已通过精确数据库检索尝试回答用户的问题，结果为空（0 条匹配）。\n"
                    f"已检索的筛选条件：{oneshot_context}\n"
                    "该检索结果是权威的。请你直接告知用户「按以上条件没有找到匹配的从者」，"
                    "列出具体条件，并建议用户尝试减少或放宽条件重新搜索。\n"
                    "禁止使用工具重新搜索相同或类似的条件——数据库已经确认没有匹配结果。"
                ),
            }
        )
