...
1. **Phase 5 P2 推进中**：P1（5/5）+ P2 LLM Retry（已完成）+ P2 前端体验补齐（已完成）。剩余 P2：异步日志非阻塞、工程自动化。详见 `需求描述.md` Phase 5 详细规划。
...
- **诸葛孔明自充数据错误**: 显示为 30%+10%+10%+30%（应为 30%+10%+10%=50%）。根因：`servants_db.json` 中 1 技能因强化保留了新旧两个版本，导致重复计算。修复方向：相同技能只按最新版本计算。待办：针对不同服务器多版本技能的选择逻辑需要产品设计
