...
class TestNaturalLanguageModeUnaffected:
    """测试旧模式（natural_language）不受 skill 模式影响。"""

    @pytest.mark.anyio
    async def test_default_mode_is_natural_language(self):
        """不传 mode 时应走旧逻辑。"""

        async def mock_nl(**kwargs):
            if kwargs.get("json_mode") is True:
                return {
                    "intent": "query_servants",
                    "conditions": {"className": "saber"},
                    "_model": "mock-nl",
                }
            return {"text": "找到了 Saber 从者。", "_model": "mock-nl"}

        with patch("server.main.chat_completion", new=AsyncMock(side_effect=mock_nl)):
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                resp = await client.post(
                    "/api/chat",
                    json={"message": "有哪些Saber"},
                )
        assert resp.status_code == 200
        data = resp.json()
        assert data["count"] > 0
        # 旧模式返回的 query 应是 conditions dict，不包含 mode 字段
        assert "mode" not in data["query"]

    @pytest.mark.anyio
    async def test_explicit_natural_language_mode(self):
        """显式传 mode=natural_language 应走旧逻辑。"""

        async def mock_nl(**kwargs):
            if kwargs.get("json_mode") is True:
                return {
                    "intent": "query_servants",
                    "conditions": {"className": "archer"},
                    "_model": "mock-nl",
                }
            return {"text": "找到了 Archer 从者。", "_model": "mock-nl"}

        with patch("server.main.chat_completion", new=AsyncMock(side_effect=mock_nl)):
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                resp = await client.post(
                    "/api/chat",
                    json={"message": "有哪些Archer", "mode": "natural_language"},
                )
        assert resp.status_code == 200
        data = resp.json()
        assert data["count"] > 0
